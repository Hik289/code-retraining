...
#SBATCH --output=compile_filter_%j.out
...
for ROUND in $(seq 1 $TOTAL_ROUNDS); do
    ...
    if [ -f "$DATA_FILE" ] && [ "$(wc -l < "$DATA_FILE")" -ge "$NUM_SAMPLES" ]; then
        ...
        echo "--- Step 1: 生成 + compile 过滤，目标 $NUM_SAMPLES 条 ---"
    ...
    python scripts/evalplus_generate.py \
        --model_path "$CKPT" \
        --dataset humaneval \
        --output_file "evalplus_results/humaneval/compile_r${ROUND}_greedy.jsonl" \
        --temperature 0.2

    rm -f "evalplus_results/humaneval/compile_r${ROUND}_greedy_eval_results.json"
    EVAL_OUT=$(evalplus.evaluate \
        --dataset humaneval \
        --samples "evalplus_results/humaneval/compile_r${ROUND}_greedy.jsonl" \
    ...
    python scripts/evalplus_generate.py \
        --model_path "$CKPT" \
        --dataset mbpp \
        --output_file "evalplus_results/mbpp/compile_r${ROUND}_greedy.jsonl" \
        --temperature 0.2

    rm -f "evalplus_results/mbpp/compile_r${ROUND}_greedy_eval_results.json"
    EVAL_OUT=$(evalplus.evaluate \
        --dataset mbpp \
        --samples "evalplus_results/mbpp/compile_r${ROUND}_greedy.jsonl" \
        --i-just-wanna-run 2>&1 | tee /dev/stderr)
...
echo ""
echo "===== [Exp A: Compile Filter] 实验完成 ====="
echo ""
echo "结果汇总："
column -t -s',' "$CSV"
